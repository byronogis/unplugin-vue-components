...
var _chunk3NAFQ3SJcjs = require('./chunk-3NAFQ3SJ.cjs');
...
function stringifyComponentInfo(filepath, { from: path, as: name, name: importName }, importPathTransform) {
  if (!name)
    return void 0;
  path = _chunk3NAFQ3SJcjs.getTransformedPath.call(void 0, path, importPathTransform);
...
function resolveOptions(options, root) {
  ...
  resolved.resolvedExtensions = _utils.toArray.call(void 0, resolved.extensions).map((i) => i.startsWith(".") ? i : `.${i}`).sort((a, b) => b.length - a.length);
  if (resolved.globs) {
    ...
    const extsGlob = resolved.resolvedExtensions.length === 1 ? resolved.resolvedExtensions : `{${resolved.resolvedExtensions.join(",")}}`;
...
async function transformComponent(code, transformer2, s, ctx, sfcPath) {
  ...
  for (const { rawName, replace } of results) {
    debug2(`| ${rawName}`);
    const name = _chunk3NAFQ3SJcjs.pascalCase.call(void 0, rawName);
    ctx.updateUsageMap(sfcPath, [name]);
    const component = await ctx.findComponent(name, "component", [sfcPath]);
    if (component) {
      const varName = `__unplugin_components_${no}`;
      s.prepend(`${_chunk3NAFQ3SJcjs.stringifyComponentImport.call(void 0, { ...component, as: varName }, ctx)};
`);
      no += 1;
      replace(varName);
    }
...
async function transformDirective(code, transformer2, s, ctx, sfcPath) {
  ...
  for (const { rawName, replace } of results) {
    debug3(`| ${rawName}`);
    const name = `${_chunk3NAFQ3SJcjs.DIRECTIVE_IMPORT_PREFIX}${_chunk3NAFQ3SJcjs.pascalCase.call(void 0, rawName)}`;
    ctx.updateUsageMap(sfcPath, [name]);
    const directive = await ctx.findComponent(name, "directive", [sfcPath]);
    if (!directive)
      continue;
    const varName = `__unplugin_directives_${no}`;
    s.prepend(`${_chunk3NAFQ3SJcjs.stringifyComponentImport.call(void 0, { ...directive, as: varName }, ctx)};
`);
    no += 1;
    replace(varName);
  }
...
function transformer(ctx, transformer2) {
  return async (code, id, path) => {
    ...
    s.prepend(_chunk3NAFQ3SJcjs.DISABLE_COMMENT);
...
var Context = (_class = class {
  ...
  transform(code, id) {
    const { path, query } = _chunk3NAFQ3SJcjs.parseId.call(void 0, id);
  ...
  setupWatcher(watcher) {
    ...
    watcher.on("unlink", (path) => {
      if (!_chunk3NAFQ3SJcjs.matchGlobs.call(void 0, path, globs))
        return;
  ...
  onUpdate(path) {
    ...
    const name = _chunk3NAFQ3SJcjs.pascalCase.call(void 0, _chunk3NAFQ3SJcjs.getNameFromFilePath.call(void 0, path, this.options));
  ...
  updateComponentNameMap() {
    ...
    Array.from(this._componentPaths).forEach((path) => {
      const name = _chunk3NAFQ3SJcjs.pascalCase.call(void 0, _chunk3NAFQ3SJcjs.getNameFromFilePath.call(void 0, path, this.options));
      if (_chunk3NAFQ3SJcjs.isExclude.call(void 0, name, this.options.excludeNames)) {
  ...
  async findComponent(name, type, excludePaths = []) {
    ...
    for (const resolver of this.options.resolvers) {
      if (resolver.type !== type)
        continue;
      const result = await resolver.resolve(type === "directive" ? name.slice(_chunk3NAFQ3SJcjs.DIRECTIVE_IMPORT_PREFIX.length) : name);
      if (!result)
        continue;
      if (typeof result === "string") {
        ...
        info = {
          as: name,
          ..._chunk3NAFQ3SJcjs.normalizeComponentInfo.call(void 0, result)
        };
      }
      if (type === "component")
  ...
  normalizePath(path) {
    return _chunk3NAFQ3SJcjs.resolveAlias.call(void 0, path, _optionalChain([this, 'access', _31 => _31.viteConfig, 'optionalAccess', _32 => _32.resolve, 'optionalAccess', _33 => _33.alias]) || _optionalChain([this, 'access', _34 => _34.viteConfig, 'optionalAccess', _35 => _35.alias]) || []);
  }
  relative(path) {
...
var unplugin_default = _unplugin.createUnplugin.call(void 0, (options = {}) => {
  ...
  const api = {
    ...
    stringifyImport(info) {
      return _chunk3NAFQ3SJcjs.stringifyComponentImport.call(void 0, info, ctx);
    }
  };
  return {
    ...
    async transform(code, id) {
      if (!_chunk3NAFQ3SJcjs.shouldTransform.call(void 0, code))
        return null;
      try {
