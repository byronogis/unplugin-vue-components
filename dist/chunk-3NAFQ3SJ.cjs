...
function getNameFromFilePath(filePath, options) {
  ...
  let filename = isEmpty(resolvedDirs) ? parsedFilePath.name : _path.basename.call(void 0, parsedFilePath.base, _optionalChain([resolvedExtensions, 'optionalAccess', _2 => _2.find, 'call', _3 => _3((ext) => parsedFilePath.base.endsWith(ext))]));
...
async function getPkgVersion(pkgName, defaultVersion) {
  try {
    ...
    if (isExist) {
      ...
      return _nullishCoalesce(_optionalChain([pkg, 'optionalAccess', _4 => _4.version]), () => ( defaultVersion));
    } else {
